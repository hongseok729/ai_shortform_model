...
# 상수 정의
DEFAULT_CONF_THRESHOLD = 0.5  # MediaPipe 기본 신뢰도 임계값
...
# 추출된 프레임에 MediaPipe 모델을 적용하여 얼굴 감지 및 바운딩 박스 표시
def process_extracted_frames(frames_dir: str, output_dir: str, shot_boundaries_path: Optional[str] = None,
    ...
    """
    추출된 프레임에 MediaPipe 모델을 적용하여 얼굴 감지 및 바운딩 박스를 그립니다.
    ...
    print("MediaPipe Face Detection 모델 로드 중...")
    mp_face_detection = mp.solutions.face_detection
    mp_drawing = mp.solutions.drawing_utils
    
    # 모델 설정 - 높은 정확도를 위해 model_selection=1 사용
    face_detection = mp_face_detection.FaceDetection(
        model_selection=1,  # 0: 근거리 감지 모델, 1: 원거리 감지 모델
        min_detection_confidence=conf_threshold
    )
    print("MediaPipe Face Detection 모델 로드 완료")
    ...
    for frame_file in frame_files:
        ...
        # BGR -> RGB 변환 (MediaPipe는 RGB 형식 사용)
        ...
        if results.detections:
            for detection in results.detections:
                ...
                # for landmark_id, landmark in enumerate(detection.location_data.relative_keypoints):
                #     landmark_px = mp_drawing._normalized_to_pixel_coordinates(
                #         landmark.x, landmark.y, iw, ih)
                #     if landmark_px:
                #         cv2.circle(frame_with_boxes, landmark_px, 2, (0, 0, 255), 2)
...
def main():
    ...
    output_dir = "../media_pipe_output_data/1"
